    ...
    void run() { // Future (outside of demo build): return array of top genes to be manipulated
        ...
        for (int i = 0; i < animals.length; i++) {
            if (i < animals.length / 3) {
                animals[i] = new Animal(i, 'c', (int) (Math.random() * width), (int) (Math.random() * height), new Genes(Genes.TEST));
            }
            else if(i < (animals.length / 3) * 2){
                animals[i] = new Animal(i, 'm', (int) (Math.random() * width), (int) (Math.random() * height), new Genes(Genes.TEST));
            }
            else{
                animals[i] = new Animal(i, 's', (int) (Math.random() * width), (int) (Math.random() * height), new Genes(Genes.TEST));
            }
        }
